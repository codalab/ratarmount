    ...
    def _createIndex(
        ...
        if fileCount == 0:
            try:
                ...
                # and set the relevant statistics (such as st_mtime) to 0.
                tarInfo = None
            ...
            fileInfo = (
                ...
                0                                           ,  # 3 data offset
                fileSize                                    ,  # 4
                tarInfo.st_mtime if tarInfo else None       ,  # 5
                tarInfo.st_mode if tarInfo else None        ,  # 6
                None                                        ,  # 7 TAR file type. Currently unused but overlaps with mode anyways
                None                                        ,  # 8 linkname
                tarInfo.st_uid if tarInfo else None         ,  # 9
                tarInfo.st_gid if tarInfo else None         ,  # 10
                False              ,  # 11 isTar
                    ...
                    if not stat.S_ISREG(fileInfo.mode) and not stat.S_ISLNK(fileInfo.mode) and fileInfo.linkname:
                        ...
                        if targetLink != filePath:
                            return self.getFileInfo(
                                os.path.join(recursiveTarFileInfo.mountPoint, targetLink), fileVersion
                            )
                    return fileInfo
                return None
